...
import yaml
from utils import ConfigManager, PerfMonitor, has_flag, parse_args_and_init_logger, Logger, create_nerf, get_random_directions, load_yaml_as_dict, LPIPS
...
def run_network(inputs, viewdirs, lightdirs, fn, embed_fn, embeddirs_fn, embedlights_fn, netchunk=1024*64):
    ...
    if lightdirs is not None:
        #input_dirs = lightdirs[:,None].expand(inputs.shape)
        input_dirs_flat = torch.reshape(lightdirs, [-1, lightdirs.shape[-1]])
        embedded_lights = embedlights_fn(input_dirs_flat)
        embedded = torch.cat([embedded, embedded_lights], -1)
...
def render(intrinsics, chunk=1024*32, rays=None, c2w=None, ndc=True,
                  near=0., far=1., c2w_staticcam=None, img_id=0, use_random_directions=False,
    ...
    if c2w is not None:
        ...
        rays_o, rays_d, rays_i = get_rays(intrinsics, c2w, img_id=img_id)
    else:
    ...
    if not use_random_directions:
        ...
        if c2w_staticcam is not None:
            ...
            rays_o, rays_d, rays_i = get_rays(intrinsics, c2w_staticcam, img_id=img_id)
    ...
    rays_i = torch.reshape(rays_i[..., :1], [-1, 1]).float()

    near, far = near * torch.ones_like(rays_d[...,:1]), far * torch.ones_like(rays_d[...,:1])
    rays = torch.cat([rays_o, rays_d, near, far], -1)
    rays = torch.cat([rays, viewdirs], -1)
    rays = torch.cat([rays, rays_i], -1)
    ...
    all_ret = batchify_rays(rays, chunk, metadata=metadata, random_directions=random_directions, background_color=background_color, **kwargs)
...
def render_path(render_poses, intrinsics, chunk, render_kwargs, gt_imgs=None, savedir=None, render_factor=0):
    ...
    for i, c2w in enumerate(tqdm(c2ws)):
        ...
        if use_fast_sampling:
            ...
            rgb, disp, acc, elapsed_time = render(intrinsics, chunk=chunk, c2w=c2w, img_id=-render_poses.shape[0]+i, **render_kwargs)[:4]
    ...
    # Logger.write('average over image batch. MSE: {:.6f}, PSNR: {:.3f}, SSIM: {:.3f}, LPIPS: {:.3f}, elapsed time: {:7.2f} ms'.format(average_mse, average_psnr, average_ssim, average_lpips, average_elapsed_time * 1000))
    #if savedir is not None:
...
def render_rays(ray_batch,
    ...
    N_rays_to_eval = N_rays

    params = {}
    ray_batch_obj = ray_utils.transform_rays(ray_batch, params, kwargs['use_viewdirs'], inverse=True)
    metadata_obj = ray_utils.transform_dirs(kwargs['metadata'], params, inverse=True)

    ray_batch_obj_to_eval = ray_batch_obj

    rays_o, rays_d = ray_batch_obj_to_eval[:, 0:3], ray_batch_obj_to_eval[:, 3:6]
    bounds = ray_batch_obj_to_eval[..., 6:8].view(-1, 1, 2)
    ...
    viewdirs, lightdirs = get_dirs(ray_batch_obj_to_eval, pts, metadata_obj, kwargs['use_viewdirs'], kwargs['use_lightdirs'], kwargs['lightdirs_method'])
    
    def query_network():
        if network_query_fn is not None:
            return network_query_fn(pts, viewdirs, lightdirs, network_fn.model_coarse if N_importance > 0 else network_fn)
        else:
            ...
            return query_multi_network(multi_network, domain_mins, domain_maxs, pts, viewdirs, lightdirs,
                position_fourier_embedding, direction_fourier_embedding, light_fourier_embedding, occupancy_grid, debug_network_color_map, use_network_jittering, random_directions, cfg)
    ...
    if N_importance > 0:
        ...
        viewdirs, lightdirs = get_dirs(ray_batch_obj_to_eval, pts, metadata_obj, kwargs['use_viewdirs'], kwargs['use_lightdirs'], kwargs['lightdirs_method'])
        #print("network_fn: ", network_fn)
        raw = network_query_fn(pts, viewdirs, lightdirs, network_fn.model_fine)
...
def train(cfg, log_path, render_cfg_path):
    ...
    elif cfg['dataset_type'] == 'osf':
        images, poses, render_poses, hwf, i_split, metadata, render_metadata = load_osf_data(
            cfg['dataset_dir'], cfg['near'], cfg['far'], cfg['spiral_radius'], cfg['half_res'], cfg['testskip'],# original load_osf
            # cfg['dataset_dir'], cfg['spiral_radius'], cfg['half_res'], cfg['testskip'], # for siggraph
            cfg['n_render_spiral'], cfg['render_spiral_angles'])
        print('Loaded osf', images.shape, render_poses.shape, hwf, cfg['dataset_dir'])
        i_train, i_val, i_test = i_split
    ...
    if cfg['dataset_type'] == 'blender' or cfg['dataset_type'] == 'nsvf' or cfg['dataset_type'] == 'osf':
    ...
    if cfg['model_type'] == 'single_network':
        ...
        network_query_fn = lambda inputs, viewdirs, lightdirs, network_fn : run_network(inputs, viewdirs, lightdirs, network_fn,
                                                                    embed_fn=embed_fn,
        ...
        if not load_from_distilled:
            ...
            model = multi_network = create_multi_network(num_networks, position_num_input_channels, direction_num_input_channels, light_num_input_channels, 4,
    ...
    if load_from_checkpoint and not load_from_distilled:
        ...
        #model.load_state_dict(cp['network_fn_state_dict'])
    ...
    for i in trange(start, N_iters):
        ...
        if use_batching:
            ...
            if N_rand is not None:
                rays_o, rays_d, rays_i = get_rays(intrinsics, torch.Tensor(pose), img_id=img_i)  # (H, W, 3), (H, W, 3)
                ...
                rays_i = rays_i[select_coords[:, 0], select_coords[:, 1]]  # (N_rand, 3)
                batch_rays = torch.stack([rays_o, rays_d, rays_i], 0)
