...
def build_occupancy_tree(cfg, log_path):
    ...
    if use_multi_network:
        position_num_input_channels, position_fourier_embedding = create_multi_network_fourier_embedding(1,
                                                                                                         pretrained_cfg[
                                                                                                             'num_frequencies'])
        direction_num_input_channels, direction_fourier_embedding = create_multi_network_fourier_embedding(1,
                                                                                                           pretrained_cfg[
        ...
        if 'model_state_dict' in cp:
            ...
            multi_network = create_multi_network(num_networks, position_num_input_channels,
            ...
            multi_network = build_multi_network_from_single_networks(single_networks,
                                                                     linear_implementation=linear_implementation).to(
                dev)
            domain_mins = torch.tensor(domain_mins, device=dev)
            domain_maxs = torch.tensor(domain_maxs, device=dev)
    else:
        # Load teacher NeRF model:
        print("Load teacher NeRF model...")
    ...
    for dim in range(3):
        first_voxel_samples.append(
            torch.linspace(first_voxel_min[dim], first_voxel_max[dim], cfg['subsample_resolution'][dim]))
    ...
    points = first_voxel_samples.unsqueeze(0).unsqueeze(0).unsqueeze(0).expand(
        occupancy_res + list(first_voxel_samples.shape))
    ...
    print("all_densities size: ", all_densities.shape)
    end = 0
    while end < total_num_voxels:
        ...
        with torch.no_grad():
            if use_multi_network:
                result = query_multi_network(multi_network, domain_mins, domain_maxs, points_subset,
                                             mock_directions_subset,
                                             position_fourier_embedding, direction_fourier_embedding, None, None, False,
                                             None, pretrained_cfg)[:, :, density_dim]
            else:
                print("Use teacher NeRF model...")
                mock_directions_subset = mock_directions_subset.unsqueeze(1).expand(points_subset.size())
                points_and_dirs = torch.cat([points_subset.reshape(-1, 3), mock_directions_subset.reshape(-1, 3)],
                                            dim=-1)
                lights_subset = torch.zeros((points_and_dirs.shape[0], 3), device=dev)
                lights_subset[:, -1] = 1
                points_and_dirs_and_lights = torch.cat([points_and_dirs, lights_subset], dim=-1)
                result = pretrained_nerf(points_and_dirs_and_lights)[:, density_dim].view(actual_batch_size, -1)
                # result = F.relu(pretrained_nerf(points_and_dirs_and_lights)[:, density_dim].view(actual_batch_size, -1))
                # print(result.max())
    ...
    Logger.write('{} out of {} voxels are occupied. {:.2f}%'.format(occupancy_grid.sum().item(), occupancy_grid.numel(),
